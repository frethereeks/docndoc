    ...
    return (
        <Link href={`/blog/${slug}`} key={id} className='block bg-white shadow-xl'>
            ...
            <div className="flex flex-col gap-2 p-3 pt-2 sm:p-4">
                <p className="text-xs md:text-sm text-text font-medium font-secondary">{moment(createdAt).fromNow()}</p>
                <div className="flex-1 space-y-2">
                    <h4 className="leading-tight text-dark text-lg sm:text-xl md:text-2xl font-primary font-bold line-clamp-2 text-ellipsis">{title}</h4>
                    <p className="text-base md:text-lg text-text font-medium font-secondary line-clamp-2 md:line-clamp-3 text-ellipsis">{content.slice(0, 100)}</p>
                </div>
