    ...
    <header className='sticky top-0 left-0 z-50 bg-white p-4'>
      <div className="container mx-auto flex justify-between gap-4">
        ...
        <nav className={`bg-white flex-1 flex flex-col md:flex-row md:justify-between transition-all duration-300 absolute top-full w-full ${navOpen ? 'left-0' : 'left-full'} md:static`}>
          <div className="flex-1 flex flex-col md:flex-row md:justify-center md:items-center md:gap-4">
            {
              headerLinks.map(el => (
          ...
          <Link key={"720z7275"} href={"/login"} className={`py-2 px-4 md:px-6 font-primary text-white relative md:rounded-sm bg-dark md:ml-auto`}>Get Started</Link>
        </nav>
